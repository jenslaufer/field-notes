...
|---|---|---|
| `2026-08-24-understanding-ai-text-watermarking` | **Transkript + zweite Quelle** | geschrieben 24.08. 02:1x, **44/44 Zitatsegmente ≥ 8 W verbatim, Exit 0** (`verify-quotes.py`, Exit-Code **ohne Pipe** gemessen). Katie allein, 206 Segmente. **Erste Notiz mit zwei Quellen, und der erste rote Gate-Lauf, der KEIN Fehler war:** vier Zitate stammen aus Anthropics eigener FAQ (`anthropic.com/news/claude-text-watermark`), die das Werkzeug nicht sehen kann — es prüft nur Transkripte, also meldete es sie als Miss. **Nicht durchgewunken:** die FAQ wurde als Quelldatei gesichert (`~/.cache/podcast-transcripts/2026-08-24-anthropic-watermark-faq.txt`) und das Gate gegen beide Quellen zusammen gefahren, bis jedes Zitat an einem echten Text hängt. Zwei echte Defekte fand der Lauf dabei: ein Zitat, dem ich `it's` → `is` verkürzt hatte, und **meine eigene Formulierung in Anführungszeichen**, die wie ein Zitat aussah. Die Notiz trennt Folge und FAQ in jedem Absatz. Eine Zahl bleibt markiert: `0100` ist der Runde-2-Vektor, im Transkript in Worten (*„our vector is zero, one, zero, zero"*, [12:29]), nicht in Ziffern. ASR stehen gelassen und aufgelöst: `Synth ID` (= SynthID), `Leichi`/`leechies`/`me cheese` (= lychee), `Duryan` (= durian), `and tropics` (= Anthropic's), `cloud selects` (= Claude), `wait the die` (= weight the die), `one bright way` (= one right way) |
| `2026-08-17-humanitys-last-exam-benchmark` | **Transkript** | geschrieben 17.08. 05:3x, **26/26 Zitatsegmente ≥ 8 W verbatim**, 0 Misses, 0 Zahlen beanstandet (`verify-quotes.py`, Exit-Code **ohne Pipe** gemessen). Erste Folge des neuen Formats *Better Know a Benchmark*, Katie allein. ASR-Verstümmelungen stehen gelassen und in der Notiz aufgelöst: `MMLEU` (= MMLU), `CLOD-3` (= Claude 3), `DeepSeq-R1` (= DeepSeek-R1), `lom-statts.com` (= llm-stats.com), `Okenneson` (= **Oganesson**), `noble gas loads` (= noble gases); die Kolibri-Anatomie im Beispielzitat ist unrettbar verstümmelt und **als gehört** wiedergegeben. **Eine Inkonsistenz der Folge selbst nicht stillschweigend geglättet:** Katie nennt den Spitzenwert beim Start zweimal verschieden (13 % bei [08:11], 14 % bei [09:27]) — beide Zahlen stehen in der Notiz |
